...
export default function CartPage() {
  ...
  if (loading) {
    return <p className="mx-auto max-w-3xl px-4 py-16 text-center text-sm text-muted">Loading your cart…</p>;
  }
  ...
  function handleCheckout() {
    // login is only required at checkout time, never just to browse/add to cart
    if (!session?.user) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      ...
      <div className="mt-6 flex flex-col gap-3">
        {cart.map((item) => (
          <div key={item.productId} className="flex items-center gap-4 rounded-lg border border-border bg-surface p-3">
            ...
            <p className="w-16 shrink-0 text-right text-sm font-semibold">${(item.price * item.quantity).toFixed(2)}</p>
